...
def get_movie_id_from_title(title):
    title = clean_title(title)  # Clean the title to match the dataset format
    matches = movies_df[movies_df["clean_title"].str.contains(title, case=False)]
    if matches.empty:
        return None  # No match found
    return matches.iloc[0]["movieId"]  # Return the first matching movieId

def find_similar_movies_by_title(title):
    # Get the movieId corresponding to the title
    movie_id = get_movie_id_from_title(title)
    if not movie_id:
        return None  # Title not found in the dataset
    
    # Perform collaborative filtering using the movieId
...
def recommend_similar_by_title():
    title = request.args.get('title', '')
    if not title:
        return jsonify({"error": "Title parameter is required"}), 400

    recommendations = find_similar_movies_by_title(title)
    if recommendations is None:
        return jsonify({"error": "Title not found"}), 404

    # Add TMDB details to the recommendations
    recommendations["tmdb_details"] = recommendations["movieId"].apply(
        lambda x: get_movie_details(links_df.loc[links_df["movieId"] == x, "tmdbId"].values[0])
    )
    recommendations["poster"] = recommendations["tmdb_details"].apply(lambda x: x.get("poster_path", ""))
    recommendations["overview"] = recommendations["tmdb_details"].apply(lambda x: x.get("overview", "No overview available"))

    # Format the output as a JSON response
    output = recommendations[["score", "title", "genres", "poster", "overview"]].to_dict(orient="records")
